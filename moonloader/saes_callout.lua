...
local subs_win_h    = 620  -- динамическая высота окна подразделений (CFG.H по умолчанию)
...
local user_avatar_url  = nil  -- Discord CDN URL аватара текущего пользователя
...
local function applyVersionInfo(data)
  ...
  if upd.required then
    screen = 'update'
    sampAddChatMessage(u8:decode('{DC143C}[CALLOUT]{FFFFFF} Ваша версия '..VERSION..' устарела и больше не поддерживается. Обновитесь: {DC143C}/callout update{FFFFFF} или вручную: '..CFG.repo_url), -1)
  elseif upd.available then
    sampAddChatMessage(u8:decode('{DC143C}[CALLOUT]{FFFFFF} Доступна новая версия {DC143C}'..data.latest..'{FFFFFF} (у вас '..VERSION..'). Команда {DC143C}/callout update{FFFFFF} — автообновление, или вручную: '..CFG.repo_url), -1)
...
local function doAutoUpdate()
  ...
  if not upd.dl_url then
    sampAddChatMessage(u8:decode('{DC143C}[CALLOUT]{FFFFFF} Ссылка на обновление недоступна. Скачайте вручную: '..CFG.repo_url), -1)
    return
  end
  if not (upd.available or upd.required) then
    sampAddChatMessage(u8:decode('{DC143C}[CALLOUT]{FFFFFF} У вас уже последняя версия ('..VERSION..').'), -1)
    return
  end
  upd.busy = true
  ...
  lua_thread.create(function()
        ...
        if size > 1000 and data:find('SAES Callout System', 1, true) then
          if size == last_size then
            ...
            if out then
              ...
              sampAddChatMessage(u8:decode('{DC143C}[CALLOUT]{FFFFFF} Обновление установлено, перезагружаю скрипт...'), -1)
              wait(300)
              thisScript():reload()
              return
            end
    ...
    upd.status = nil
    os.remove(staging)
    sampAddChatMessage(u8:decode('{DC143C}[CALLOUT]{FFFFFF} Не удалось автообновиться. Скачайте вручную: '..CFG.repo_url), -1)
  end)
end
...
local function drawToken()
  local sw, sh = getScreenResolution()
  applyWindowPos()
  imgui.SetNextWindowSize(imgui.ImVec2(420, 0))
  local flags = imgui.WindowFlags.NoResize + imgui.WindowFlags.NoScrollbar
...
local function processNextHttp()
  ...
  lua_thread.create(function()
      ...
      local ok = pcall(function()
        t = effil.thread(function(method, url, auth, ct, cl, body)
        ...
        end)(method, url, auth, ct, cl, body)
...
fetchUserAvatar = function()
  if not auth or user_avatar_url ~= nil then return end
...
local function drawSubdivisions()
  local sw, sh = getScreenResolution()
  applyWindowPos()
  imgui.SetNextWindowSize(imgui.ImVec2(CFG.W, subs_win_h))
  local flags = imgui.WindowFlags.NoResize + imgui.WindowFlags.NoScrollbar
              + imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoTitleBar
  ...
  elseif sub_view == 'factions' then
    ...
    if #factions_list == 0 then
      ...
      -- База расчёта — ПОСТОЯННАЯ проектная высота CFG.H, а не текущая (уже ужатая)
      -- высота окна: иначе ужатие окна уменьшало бы avail_h и число карточек «плыло» бы
      local avail_h = math.max(row_h, CFG.H - cards_top - foot_h)
      ...
      -- Ужать окно под контент: footer садится сразу под списком, без пустоты снизу.
      -- Высоту фиксируем по ПОЛНОЙ странице (cards_shown == FPER), чтобы окно не
      -- «дёргалось» при переходе на неполную последнюю страницу.
      if not show_pg or cards_shown >= FPER then
        subs_win_h = imgui.GetCursorPosY() + foot_h
      end
    end
...
local function drawCallout()
  local sw, sh = getScreenResolution()
  applyWindowPos()
  imgui.SetNextWindowSize(imgui.ImVec2(CFG.W, 510))
  local flags = imgui.WindowFlags.NoResize + imgui.WindowFlags.NoScrollbar
              + imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoTitleBar
...
local function drawSuccess()
  local sw, sh = getScreenResolution()
  applyWindowPos()
  imgui.SetNextWindowSize(imgui.ImVec2(380, 0))
  local flags = imgui.WindowFlags.NoResize + imgui.WindowFlags.NoScrollbar
              + imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoTitleBar
...
local function drawUpdate()
  applyWindowPos()
  imgui.SetNextWindowSize(imgui.ImVec2(440, 0))
  local flags = imgui.WindowFlags.NoResize + imgui.WindowFlags.NoScrollbar
              + imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoTitleBar
...
imgui.OnFrame(
  function() return visible[0] or is_closing end,
  function(self)
    ...
    -- Плавное закрытие: применяем Alpha ко всему UI
...
function main()
  ...
  loadConfig()
  if auth then fetchUserAvatar() end  -- fetchUserAvatar определена к этому моменту
  ...
  if not doesFileExist(CFG.cfg_file) then
    sampAddChatMessage(u8:decode('{DC143C}[CALLOUT]{FFFFFF} Вы можете настроить горячую клавишу открытия меню в настройках.'), -1)
  end
